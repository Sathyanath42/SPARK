...
# load the class labels from disk, split them on the basis of space
rows = open(args["labels"]).read().strip().split("\n")
classes = [r[r.find(" ") + 1:].split(",")[0] for r in rows]

# performing mean subtraction to normalise the input
...
for (i, idx) in enumerate(idxs):
	...
	if i == 0:
    	#text = ("Label:",classes[idx]," probability:",preds[0][idx] * 100)
